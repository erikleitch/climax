...
void plotPowerSpectrum(std::vector<double>& yarr, bool doLine);
...
int Program::main()
{
  ...
  if(!manual) {
	...
	input[i] = sin(2*i*M_PI/10);
	//	input[i] *= (apod ? apodizationCoefficient(n, i) : 1.0);
	dft.pushSample(input[i]);
      }
    ...
    COUT("Plotting array: " << input[0] << " " << input[n-1]);
    plot(input);

    dft.fillInputArray(&input[0]);
...
void plotPowerSpectrum(std::vector<double>& yarr, bool doLine)
{
  ...
  dft.setTimeRes(tVal);
  dft.setAverage(false);

  std::vector<double> x(yarr.size());
  double dx = 1.0;

  try {
    dx = dft.getFrequencyResolution().Hz();
  } catch(...) {
  }
  ...
  for(unsigned i=0; i < yarr.size(); i++) {
    ...
    COUT("About to push sin sample: " << yarr[i]);

    x[i] = i*dx;
    dft.pushSample(sin(2*M_PI*x[i]/100));
  }
