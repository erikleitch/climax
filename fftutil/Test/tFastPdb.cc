...
KeyTabEntry Program::keywords[] = {
  ...
  { "src",       "MARS", "s", "Source to match (comma-separated list for more than one)"},
...
std::vector<string> getSources(std::string sources)
{
  String sourcesStr(sources);
  std::vector<string> sourceVec;

  String nextSrc;
  do {
    nextSrc = sourcesStr.findNextStringSeparatedByChars(",", true);

    if(!nextSrc.isEmpty()) {
      COUT("Pushing back: '" << nextSrc << "'");
      sourceVec.push_back(nextSrc.str());
    }

  } while(!nextSrc.isEmpty());
...
void listTrialsMatchingSource(std::vector<EmlProject>& projects, std::string source, bool doFreq, double freq)
{
  ...
  std::vector<string> sources = getSources(source);
  ...
  for(unsigned iProj=0; iProj < projects.size(); iProj++) {
	...
	for(unsigned iTrial=0; iTrial < subblock.trials_.size(); iTrial++) {
	  ...
	  if(freqMatch) {
	    ...
	    //------------------------------------------------------------
	    // Iterate over all sources in this trial
	    //------------------------------------------------------------

	    unsigned nSrcMatched=0;
	    for(unsigned iSrc=0; iSrc < trial.sources_.size(); iSrc++) {
	      EmlSrc& src = trial.sources_[iSrc];
	      String sourceStr(src.name_);

	      //------------------------------------------------------------
	      // Iterate over all sources we are trying to match
	      //------------------------------------------------------------

	      for(unsigned iMatch=0; iMatch < sources.size(); iMatch++) {
		if(sourceStr.contains(sources[iMatch])) {
		  nSrcMatched++;
		}
	      }
	    ...
	    //------------------------------------------------------------
	    // If we found a match for all sources we are trying to
	    // match, then add this trial
	    //------------------------------------------------------------

	    if(nSrcMatched == sources.size()) {
  ...
  for(unsigned i=0; i < trials.size(); i++) {
    unsigned index = trialMap[sortedDates[i]];
    COUT(setw(lenMax+5) << std::left << trials[index] << " " << std::left << startDates[index] << " - " << stopDates[index] << " " << srcs[index]);
  }
}
