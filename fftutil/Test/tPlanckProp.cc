...
int Program::main()
{
  Angle sig(Program::getStringParameter("sig"));
...
void testPlanck(Angle sigma)
{
  unsigned npix = 512;
  Image image;
  image.resize(npix,npix);

  Angle size;
  size.setDegrees(0.75);

  image.xAxis().setAngularSize(size);
  image.yAxis().setAngularSize(size);

  double resDeg = 0.75/npix;
  double sigDeg = sigma.degrees();

  for(unsigned i=0; i < npix; i++)
    for(unsigned j=0; j < npix; j++) {
      unsigned ind = npix*j + i;
      double dx = ((double)(i)-npix/2) * resDeg;
      image.data_[ind] = exp(-(dx*dx)/(2*sigDeg*sigDeg));
    }

  image.hasData_ = true;
  image.display();

  image.writeToFitsFile("planck.fits");
}
