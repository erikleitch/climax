...
void imDisp();
void testResize();
void testConvolution(std::string file, double sigma);
void testAxes();
void testCS(double period, double deg);
void testPB(double freqGHz);
void testPBConv(double freqGHz);
void testDft();
void testStats(std::string file, double min, unsigned nbin);
void printStats(Image& image, double min, unsigned nbin);
void testDft2();
...
void plotGauss(double xmin, double xmax, double sig, double norm);
...
int Program::main()
{
...
#if 0
  Image image;
  image.createGaussianImage(32,32,10);
  image.display();
#endif

  imDisp();

  //  testStats(file, min, Program::getIntegerParameter("nbin"));
  ...
  //  testConvolution(file, Program::getDoubleParameter("sigma"));
...
void testConvolution(std::string file, double sigma)
{
  ...
  unsigned xnpix = image.xAxis().getNpix();
  unsigned ynpix = image.yAxis().getNpix();

  image2.createGaussianImage(xnpix, ynpix, sigma);
...
void testStats(std::string file, double min, unsigned nbin)
{
  ...
  double sizeInArcsec = 1024 * 4;
  double res = (0.8 * 3600)/256;
  unsigned newNpix = sizeInArcsec/32;
  ...
  printStats(image2, min, nbin);
  printStats(image, image.min(), nbin);
}
...
void printStats(Image& image, double min, unsigned nbin)
{
  ...
  PgUtil::histogram(&data, nbin);

  std::vector<double> xh;
  std::vector<double> yh;
  Stats::histogram(data, nbin, xh, yh);

  PgUtil::setOverplot(true);
  PgUtil::setWin(false);
  PgUtil::setTraceColor(6);

  plotGauss(Stats::min(data), Stats::max(data), Stats::rms(data), Stats::max(yh));
}
...
void plotGauss(double xmin, double xmax, double sig, double norm)
{
  ...
  for(unsigned i=0; i < npt; i++) {
    x[i] = xmin + dx*i;
    y[i] = norm*exp(-x[i]*x[i]/(2*sig*sig));
  }
...
void imDisp()
{
  ...
  image.initialize(size, size, 512, 512);

  Angle xsigma, ysigma;
  xsigma.setDegrees(0.2);
  ysigma.setDegrees(0.1);
  image.createGaussianImageFullSpecification(1.0, xsigma, ysigma, rot);
