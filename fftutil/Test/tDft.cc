...
#if 0
      input[i] = sin(i * 2*M_PI/period1) + ratio*sin(i * 2*M_PI/(period2)) + amp*(((double)rand())/RAND_MAX - 0.5) + floor;
#else
      input[i] = Sampler::generateGaussianSample(Program::getDoubleParameter("sigma"));
#endif
      dft.pushSample(input[i]/sqrt((double)n));
    }

  COUT("RMs = " << Stats::rms(input));

  plot(input);
  std::vector<double> y = dft.abs();
  plot(y, dft.getFrequencyResolution().Hz());

  COUT("Input array has " << n << " elements.  Output has " << y.size() << "y[0] = " << y[0] << " mean = " << Stats::mean(y) << " sum = " << Stats::sum(y));
}
