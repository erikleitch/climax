...
void Logger::write(const std::string& message, bool isError, bool dispatch, bool log)
{
  ...
  int incr   = NET_LOG_MAX - (prefix_.length()+1);
...
{
  if((prefix.length()) >= NET_LOG_MAX)
    ThrowError("Logger::setPrefix(): Prefix is too long");
...
LOG_HANDLER_FN(Logger::dontPrint)
{
