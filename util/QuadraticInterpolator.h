#ifndef GCP_UTIL_QUADRATICINTERPOLATOR_H
#define GCP_UTIL_QUADRATICINTERPOLATOR_H
...
namespace gcp {
  namespace util {
    ...
    public:
      ...
      static double directEval(double x0, double y0, double x1, double y1, double x2, double y2, double x)
      {
	double p = (y2-y1) / (x2-x0) / (x2-x1);
	double q = (y1-y0) / (x2-x0) / (x1-x0);
	double a = p - q;
	double b = q * (x2 + x1) - p * (x1 + x0);
    ...
    protected:
       ...
       *  @throws Exception
       */
      QuadraticInterpolator(bool relativeX=false);
    ...
    private:
      ...
      bool relativeX_;         // If true, calculations will be done
                               // relative to the minimum x value in this container
