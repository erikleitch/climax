#define __FILEPATH__ "util/SignalTask.cc"
...
#define SIGNALTASK_IO_SIGNAL SIGUSR1
 ...
 */
void SignalTask::SignalHandler::
addHandler(SIGNALTASK_HANDLER_FN(*handler), void* args)
{
...
removeHandler(SIGNALTASK_HANDLER_FN(*handler))
{
  ...
  vector<SignalTask::SignalHandler::HandlerPair>::iterator hand;

  for(hand=handlers_.begin(); hand != handlers_.end(); hand++) {
			      ...
			      SIGNALTASK_HANDLER_FN(*handler))
{
  ...
  for(unsigned itimer=0; itimer < timers_.size(); itimer++) {
    ...
    if(timers_[itimer]->sigNo_ == sigNo) 
      ThrowError("A timer is already installed for signal: " << sigNo);

    if(timers_[itimer]->name_ == name) 
      ThrowError("A timer with name: " << name << " is already installed");
  ...
  for(unsigned ihand=0; ihand < signalHandlers_.size(); ihand++)
    if(signalHandlers_[ihand]->sigNo_ == sigNo) 	
      ThrowError("A handler is already installed for signal: " << sigNo)
 ...
 */
void SignalTask::
addHandler(string name, SIGNALTASK_HANDLER_FN(*handler), void* args)
{
 ...
 */
void SignalTask::
installSignal(int sigNo, SIGNALTASK_HANDLER_FN(*handler), void* args)
{
  // Now check if a handler already exists for this signal.
 ...
 */
void SignalTask::
sendAddHandlerMsg(int sigNo, SIGNALTASK_HANDLER_FN(*handler), bool add)
{
  SignalTaskMsg msg;
 ...
 */
void SignalTask::
addHandler(int sigNo, SIGNALTASK_HANDLER_FN(*handler), void* args)
{
  COUT("Inside addHandler");
...
void SignalTask::run()
{
  ...
  while(true) {
	...
	// Rearm any periodic timer associated with this signal

	signalHandlers_[isig]->reArm();

	// Iterate through any handlers which have been installed for
	// this signal
	
	for(unsigned ihand=0; ihand < signalHandlers_[isig]->handlers_.size(); 
	    ihand++)
	  ...
	  if(signalHandlers_[isig]->handlers_[ihand].handler_ != 0) {
	    ...
	    SIGNALTASK_HANDLER_FN(*handler) = 
	      signalHandlers_[isig]->handlers_[ihand].handler_;
	    ...
	    try {
	      ...
	      ReportError("====== Exception: ======" << err.what() << endl
...
void SignalTask::serviceMsgQ()
{
  ...
  if(fd < 0)
    ThrowError("Received invalid file descriptor");
...
void SignalTask::processTaskMsg(bool* stop)
{
  ...
  default: 
    ...
    ReportError("Unknown message type " << msg.type);
    break;
