...
{
  COUT("Inside generateSources: fluxMin = " << fluxMin);
...
/**.......................................................................
 * Return the confusion limit for a given solid angle.
 *
 * The confusion limit is defined by the flux at which the integrated
 * number of sources over the given solid angle is 1:
 *
 * 1 = int(dNdS) * omega = k_ * area/au_ * pow(f/fu_, 1-gamma) / (gamma-1);
 */
Flux PtSrcGen::confusionLimit(SolidAngle area)
{
  double arg = (gamma_ - 1.0) / (k_ * (area.sr()/au_.sr()));
  double rat = pow(arg, 1.0/(1.0-gamma_));

  Flux flux;
  flux.setJy(fu_.Jy() * rat);
