...
void FitsUvfReader::getPars()
{
  ...
  for(int iPar=0; iPar < nPar_; iPar++) {
	...
	wUnit_ = SECONDS;
      ...
      ibaseline_ = iPar;
    } else if(strstr(pars_[iPar].type_.c_str(), "SOURCE") != 0) {
      source_ = &pars_[iPar];
      isource_ = iPar;
...
void FitsUvfReader::readGroup(long group, Vis& vis)
{
  ...
  //------------------------------------------------------------
  // Source may or not may not be present
  //------------------------------------------------------------

  if(isource_ < 0) {
    vis.source_ = 0;
  } else {
    vis.source_ = pars[isource_];
  }
}
...
void FitsUvfReader::readPrimaryData(long group, Vis& vis)
{
  ...
  for(unsigned iDec=1; iDec <= dec_->n_; iDec++) {
    double dec = dec_->refVal_ + (iDec - dec_->refPix_) * dec_->delta_;

    for(unsigned iRa=1; iRa <= ra_->n_; iRa++) {
      double ra = ra_->refVal_ + (iRa - ra_->refPix_) * ra_->delta_;

      for(unsigned iIf=1; iIf <= if_->n_; iIf++) {
	double ifVal = if_->refVal_ + (iIf - if_->refPix_) * if_->delta_;

	for(unsigned iFreq=1; iFreq <= freq_->n_; iFreq++) {
	  double freq = freq_->refVal_ + (iFreq - freq_->refPix_) * freq_->delta_;

	  for(unsigned iStokes=1; iStokes <= stokes_->n_; iStokes++) {
	    double stokes = stokes_->refVal_ + (iStokes - stokes_->refPix_) * stokes_->delta_;
...
double FitsUvfReader::freqDelta()
{
  return freq_->delta_;
}
...
void FitsUvfReader::setFreqs(double* vals)
{
  for(unsigned iFreq=0; iFreq < freq_->n_; iFreq++) 
    vals[iFreq] = freq_->refVal_ + ((iFreq+1) - freq_->refPix_) * freq_->delta_;
}
...
void FitsUvfReader::setIfs(double* vals)
{
  for(unsigned iIf=0; iIf < if_->n_; iIf++) 
    vals[iIf] = if_->refVal_ + ((iIf+1) - if_->refPix_) * if_->delta_;
}

void FitsUvfReader::setStokes(double* vals)
{
  for(unsigned iStokes=0; iStokes < stokes_->n_; iStokes++) 
    vals[iStokes] = stokes_->refVal_ + ((iStokes+1) - stokes_->refPix_) * stokes_->delta_;
}
