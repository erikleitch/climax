...
unsigned int Coord::getIndex(unsigned iAxis)
{
  ...
  if(ind_.size() < iAxis+1) {
    ThrowError("Invalid axis requested for axis: " << iAxis);
  }

  if(!initialized_[iAxis]) {
    ThrowError("No corrdinate was specified for axis: " << iAxis);
  }
 ...
 * Print a Coord object onto a stream
 */
ostream& gcp::util::operator<<(std::ostream& os, Coord& coord)
{
