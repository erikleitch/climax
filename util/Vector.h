#ifndef GCP_UTIL_VECTOR_H
#define GCP_UTIL_VECTOR_H
...
namespace gcp {
  namespace util {
	...
	return data_.size();
      ...
      void operator=(std::vector<type>& vec);
      ...
      /**.......................................................................
       * Multiply a vector by a like type
       */
     ...
     */
    template<class type>
      void Vector<type>::operator=(const std::vector<type>& vec)
      {
	*this = (std::vector<type>&) vec;
      }

    template<class type>
      void Vector<type>::operator=(std::vector<type>& vec)
      {
	...
	if(i > data_.size()-1) {
	  ThrowError("Vector has no element: " << i);
	}
    ...
    template<class type>
      ...
      {
	if(data_.size() == 0)
	  ThrowError("Vector is zero length");

	type maxVal=data_[0];
	for(unsigned i=0; i < data_.size(); i++) {
	  maxVal = maxVal > data_[i] ? maxVal : data_[i];
	}
    ...
    template<class type>
      const Vector<double> 
      Vector<type>::operator/(type factor)
      {
	if(data_.size()==0) {
    ...
    template<class type>
      void Vector<type>::operator/=(type factor)
      {
	if(data_.size()==0) {
	  ThrowError("Vector has size zero");
	}
	...
	if(data_.size()==0) {
	  ThrowError("Vector has size of zero");
	}
...
#endif // End #ifndef GCP_UTIL_VECTOR_H
