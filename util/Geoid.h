  ...
  namespace util {
	...
	*this = (Lla&)lla;
    ...
    public:
      ...
      PolarLengthVector geodeticLlaAndHaDecToAzElTest(Lla& geodeticLla, 
						      HourAngle& ha, Declination& declination, 
						      SrcDist type=DIST_INF, Length distance=Length(Length::Meters(), 0.0));
      ...
      LengthTriplet geodeticLlaEnuAndHaDecToUvw(Lla& geodeticLla, 
						LengthTriplet enu,
						HourAngle ha, 
						Declination declination);
      ...
      //-----------------------------------------------------------------------
      // Return the UVW coordinates of the specified (HA, Dec) point,
      // relative to the passed XYZ coordinate
      //-----------------------------------------------------------------------

      LengthTriplet haDecAndXyzToUvw(HourAngle ha, 
				     Declination declination,
				     LengthTriplet xyz);
    ...
    public:
      ...
      gcp::util::Matrix<double> getEnuToEnhRot(Angle latDiff);

      gcp::util::Matrix<double> getEnhToXyzLatRot(Angle geocentricLatitude);
      gcp::util::Matrix<double> getXyzToEcfLngRot(Angle longitude);

      gcp::util::Matrix<double> getXyzToUvwHaRot(HourAngle ha);
      gcp::util::Matrix<double> getXyzToUvwDecRot(Declination dec);
					     
      //------------------------------------------------------------
      // Convenience variables for faster computation
      //------------------------------------------------------------

      LengthTriplet xyzObs_;
      Lla geocentricLla_;
      LengthTriplet enu_;
      Length norm_;
      PolarLengthVector srcAzEl_;
