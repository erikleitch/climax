...
unsigned VisIo::getNumberOfTelescopes()
{
 ...
 */
void VisIo::setTelescopeLocations(std::vector<LengthTriplet>& locations)
{
  if(locations.size() == 0)
...
std::vector<LengthTriplet> VisIo::getTelescopeLocations()
{
  return locations_;
}
...
void VisIo::setInstrumentName(std::string instrument) 
{
  instrument_ = instrument;
  instrumentPar_.update();
} 

std::string VisIo::getInstrumentName()
{
  if(instrumentPar_.isSet()) {
    return instrument_;
  } else {
...
std::string VisIo::getTelescopeName()
{
  if(telescopePar_.isSet()) {
    return telescope_;
  } else {
...
std::string VisIo::getSourceName()
{
...
void VisIo::setDec(Declination dec) 
{
  dec_ = dec;
  decPar_.update();
}

Declination VisIo::getDec()
{
  if(decPar_.isSet()) {
    return dec_;
  } else {
...
void VisIo::setEquinox(double equinox)
{
 equinox_ = equinox;
 equinoxPar_.update();
}

double VisIo::getEquinox()
{
  return equinox_;
}
...
void VisIo::setDecApp(Declination dec) 
{
  decApp_ = dec;
  decAppPar_.update();
}

Declination VisIo::getDecApp()
{
  if(decAppPar_.isSet()) {
    return decApp_;
  } else {
...
void VisIo::setDDecApp(Declination dec) 
{
  dDecApp_ = dec;
  dDecAppPar_.update();
}
...
void VisIo::setDecRef(Declination dec)
{
  decRef_ = dec;
  decRefPar_.update();
}
...
void VisIo::setLinelengthPhases(std::vector<Angle>& phases)
{
  checkNumberOfTelescopes(phases.size());
  llPhases_ = phases;
  llPhasesPar_.update();
}

void VisIo::setLinelengthDelays(std::vector<Delay>& delays)
{
  checkNumberOfTelescopes(delays.size());
  llDelays_ = delays;
  llDelaysPar_.update();
}
...
{
  relativeHumidity_ = relativeHumidity * 100;
  relativeHumidityPar_.update();
}

void VisIo::setPurpose(std::string purpose) 
{
  purpose_ = purpose;
  purposePar_.update();
}
