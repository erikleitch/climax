...
void Geoid::setDatum(GeoDatum datum, Length sphericalRadius)
{
  ...
  case DATUM_SPHERE:
    //    COUT("Setting datum to sphere with radius = " << sphericalRadius);
 ...
 * by geodeticLla.
 *
 * For coordinate system reference, and analytic expansions, see:
 *
...
std::ostream& gcp::util::operator<<(std::ostream& os, const Lla& lla)
{
  return operator<<(os, (Lla&) lla);
}

std::ostream& gcp::util::operator<<(std::ostream& os, Lla& lla)
{
...
std::ostream& gcp::util::operator<<(std::ostream& os, const LengthTriplet& triplet)
{
  return operator<<(os, (LengthTriplet&)triplet);
}

std::ostream& gcp::util::operator<<(std::ostream& os, LengthTriplet& triplet)
{
...
std::ostream& gcp::util::operator<<(std::ostream& os, GeoDatum datum)
{
  switch (datum) {
  case  DATUM_SPHERE:
    os << "SPHERE";
...
GeoDatum gcp::util::stringToDatum(std::string str)
{
  if(str == "SPHERE") {
    return DATUM_SPHERE;
...
std::ostream& gcp::util::operator<<(std::ostream& os, GeoCoordSystem coordSystem)
{
  switch (coordSystem) {
  case  COORD_UTM:
    os << "UTM";
 ...
 * and UVW
 */
gcp::util::Matrix<double> Geoid::getXyzToUvwDecRot(Declination dec)
{
  gcp::util::Matrix<double> decRot(3,3);
 ...
 */
gcp::util::Matrix<double> Geoid::getXyzToUvwHaRot(HourAngle ha)
{
  gcp::util::Matrix<double> haRot(3,3);
 ...
 * coordinates to XYZ
 */
gcp::util::Matrix<double> Geoid::getEnhToXyzLatRot(Angle geocentricLatitude)
{
  gcp::util::Matrix<double> latRot(3,3);
 ...
 * coordinates
 */
gcp::util::Matrix<double> Geoid::getXyzToEcfLngRot(Angle longitude)
{
 ...
 */
gcp::util::Matrix<double> Geoid::getEnuToEnhRot(Angle latDiff)
{
  gcp::util::Matrix<double> decRot(3,3);
...
LengthTriplet 
Geoid::haDecAndXyzToUvw(HourAngle ha, 
 ...
 */
LengthTriplet 
Geoid::geodeticLlaEnuAndHaDecToUvw(Lla& geodeticLla, LengthTriplet enu,
				   HourAngle ha, Declination declination) 
{
  LengthTriplet xyz = geodeticLlaAndEnuToXyz(geodeticLla, enu);
  return haDecAndXyzToUvw(ha, declination, xyz);
}
 ...
 */
PolarLengthVector
Geoid::geodeticLlaAndHaDecToAzElTest(Lla& geodeticLla, 
				 HourAngle& ha, Declination& declination, 
				 SrcDist type, Length distance)
{
  //------------------------------------------------------------
  ...
  xyzObs_ = geodeticLlaAndEnuToXyz(geodeticLla);
  geocentricLla_ = geodeticLlaAndEnuToGeocentricLla(geodeticLla);

  //------------------------------------------------------------
  // Normalize to unit length
  //------------------------------------------------------------

  Vector<double> xyzObsDir = xyzObs_ / xyzObs_.magnitude();

  //------------------------------------------------------------
  // Factor of R_obs/R_src, or 0 if source is infinitely far away
  //------------------------------------------------------------

  double fac = (type==DIST_INF ? 0.0 : xyzObs_.magnitude() / distance);
  ...
  Matrix<double> enhToXyzRot = getEnhToXyzLatRot(geocentricLla_.latitude_);
  Matrix<double> xyzToEnhRot = enhToXyzRot.inverse();
  Vector<double> enhSrcDir   = (xyzToEnhRot * topoXyzSrcDir);
  ...
  Matrix<double> eastRot   = getEnuToEnhRot(geodeticLla.latitude_ - geocentricLla_.latitude_).inverse();
  Vector<double> enuSrcDir = eastRot * enhSrcDir;
  ...
  norm_.setMeters(1.0);
  enu_ = enuSrcDir * norm_;

  srcAzEl_ = enuToAzEl(enu_);
  ...
  if(!isfinite(srcAzEl_.az_.radians()) || !isfinite(srcAzEl_.el_.radians())) {
    ThrowError("Az/El position is not defined for Ha = " << ha
	       << ", Dec = " << declination
	       << " at geodetic latitude = " << geodeticLla.latitude_ << endl);
  } 
 ...
 */
std::ostream& gcp::util::operator<<(std::ostream& os, const PolarLengthVector& angles)
{
 ...
 * Print a polar length vector
 */
std::ostream& gcp::util::operator<<(std::ostream& os, PolarLengthVector& angles)
{
 ...
 */
Vector<Length> gcp::util::operator*(Matrix<double>& mat, LengthTriplet& triplet)
{
