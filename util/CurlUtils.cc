...
const CURLcode CurlUtils::performRequest(CURL* ctx, bool printStats)
{
  ...
  if(printStats) {
    if(rc != CURLE_OK) {
      ...
      CTOUT("Error from cURL: " << curl_easy_strerror( rc ));
...
std::string CurlUtils::getUrl(std::string url, bool printStats)
{
  ...
  // Eml added cleanup call Wed 24 Feb to see if that is responsible
  // for cURL failures in comomunciating with the weather station
