...
/* Open the file if necessary. */
  ...
  *iostat = 0;
  ...
  if(!strcmp(keyword,".")){
    ...
    item->bsize = BUFSIZE;
    item->io[0].buf = (char*)Malloc(BUFSIZE);
    if(BUFDBUFF)item->io[1].buf = (char*)Malloc(BUFSIZE);
...
------------------------------------------------------------------------*/
{
  ...
  while(hreadb_c(ihandle,s,offset,CACHE_ENT,iostat),!*iostat){
    ...
    item->io[0].state = IO_VALID;
    item->io[0].buf = (char*)Malloc(item->size);
