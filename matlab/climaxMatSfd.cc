 ...
 * Use like:
 *
 * d=szaMatEqGal(galLat, galLng, nu)
 *
 */
		 ...
		 int nrhs, const mxArray *prhs[])
{
  try {
    ...
    if(nrhs < 4) {
      std::cerr << "Usage: d = szaMatSfd(l(rad), b(rad), freq(GHz), units(string))" << std::endl << std::endl
    ...
    std::string pathStr("/data/szadaq/dust/maps/");

    if(nrhs == 5) 
      pathStr = MexParser::getString(prhs[4]);
    ...
    predict_thermal(Inu, nGal, l, b, freq, 
		    (char*)pathStr.c_str(), "I4096", (char*)units.c_str(), 8, 1, 1, 0);
