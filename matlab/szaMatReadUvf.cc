 ...
 * Use like:
 *
 * d=szaMatReadUvf({'array.frame.record','corr.band0.usb[0][0]',
 *                     'antenna*.tracker.actual double', 'antenna*.tracker.source string'},
 *                     '06-jan-2005:15','06-jan-2005:16',
 *                     '/data/szadaq/arc','/home/szadaq/carma_unstable/sza/array/conf/cal');
 *
 */
...
{
  sza::util::Logger::installStdoutPrintFn(MexHandler::stdoutPrintFn);
  sza::util::Logger::installStderrPrintFn(MexHandler::stderrPrintFn);
  
  sza::util::ErrHandler::installThrowFn(MexHandler::throwFn);
  sza::util::ErrHandler::installReportFn(MexHandler::reportFn);
  sza::util::ErrHandler::installLogFn(MexHandler::logFn);
...
mxArray* readUvfFile(const mxArray* prhs[])
{
  ...
  sza::util::FitsUvfReader reader(fName);
...
void addTables(mxArray* hdr, std::string fileName)
{
  ...
  *ntable = tableReader.nTable();
  ...
  for(unsigned iTable=0; iTable < tableReader.nTable(); iTable++) {
    tableReader.getNextTableInfo();
    addTable(tables, tableReader, tableReader.tableName(), iTable);
    COUT("Adding table: " << tableReader.tableName());
  }
